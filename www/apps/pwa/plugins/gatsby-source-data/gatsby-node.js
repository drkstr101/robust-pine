...
exports.sourceNodes = (props, pluginOptions = {}) => {
  ...
  const createSiteDataFromFilesPartial = partial(createSiteDataFromFiles, {
    dataPath,
    createNode,
    createContentDigest,
    reporter,
...
async function readDirRecursively(dir, options) {
  const rootDir = get(options, "rootDir", dir)
  ...
  return chain(recFiles).compact().flatten().value()
}
